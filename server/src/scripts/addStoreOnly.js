...
/**
 * Add a new store without admin user
 * Usage: node src/scripts/addStoreOnly.js <storeNumber> <storeName> <storeAddress> [storePhone] [storeEmail]
 */
async function addStoreOnly() {
  try {
    ...
    if (args.length < 3) {
      ...
      console.error('node src/scripts/addStoreOnly.js <storeNumber> <storeName> <storeAddress> [storePhone] [storeEmail]');
      process.exit(1);
    }
